...
class LineIndicator @JvmOverloads constructor(
	...
	override fun onInflate(count: Int) {
		...
		val groupViewParams = LayoutParams(LayoutParams.MATCH_PARENT, 5.dp.toInt())
		groupViewParams.gravity = Gravity.BOTTOM
		groupViewParams.setMargins(5.dp.toInt())
		var rect: View
		repeat(count) {
			rect = View(context)
			if (it == 0) {
				rect.setBackgroundResource(R.drawable.shape_indicator_line)
			} else {
			...
			if (it == 0) {
				rectParams.setMargins(0, 0, 2.dp.toInt(), 0)
			} else if (it == count - 1) {
				rectParams.setMargins(2.dp.toInt(), 0, 0, 0)
			} else {
				rectParams.setMargins(2.dp.toInt(), 0, 2.dp.toInt(), 0)
			}
	...
	override fun onPointChange(current: Int, count: Int) {
		...
		groupView.forEachIndexed { index, view ->
			if (index <= current) {
				view.setBackgroundResource(R.drawable.shape_indicator_line)
			} else {
